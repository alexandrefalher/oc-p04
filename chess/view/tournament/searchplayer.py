...
class Searchplayer(View):
    def __init__(self, model: DataModel):
        ...
        self.__players_filtered: List[Player] = []
        self.__player_index: int = model.get("tournament_player_index")

    def generate(self, model: DataModel) -> str:
        view: str = ""
        for player in self.__players:
            if not Utils.contains(self.__tournament.players, lambda player_id: player_id == player.id):
                self.__players_filtered.append(player)
                continue
        for player in self.__players_filtered:
            birth_date = Utils.date_to_date_str(player.birth_date)
    ...
    def flow(self, user_input: Any, model: DataModel) -> Request:
        ...
        if 1 <= int(user_input) and int(user_input) < len(self.__actions):
            selected_player: Player = self.__players_filtered[int(user_input) - 1]
